...
ul.addEventListener('click', function(e) {
    if(e.target.classList[1] === "fa fa-pencil-square-o"){
        var parentPar = e.target.paretNode;
        ...
        input.addEventListener('keypress', function(e) {
            if(e.keyCode === 13){
                if(input.value !== ''){
                console.log('Youu pressed Enter key');
...
hideItem.addEventListener('click', function(){
    ...
    if(hideItem.checked){
        ...
        label.textContent = 'Hide nptes';
        ul.style.display = 'block';
    }
});

var searchInput = document.getElementById('#search-note input');
